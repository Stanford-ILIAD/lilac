...
LILAC from Proprioceptive + Object State; leverages precomputed language embeddings from a BERT-style LM (pooled),
...
class LILAC(LightningModule):
    ...
    def build_model(self) -> None:
        ...
        # Learned "No-Context" Vector (acts to replace state when alpha = 0)
    ...
    def decoder(self, state: torch.Tensor, lang: torch.Tensor, alpha: torch.Tensor, z: torch.Tensor) -> torch.Tensor:
        ...
        # Latent Actions --> Normalize state & no-context embeddings to be unit-norm (because of gating)
        h_state, h_no_context = F.normalize(h_state, dim=1), F.normalize(self.h_no_context, dim=1)

        # LILAC-only :: Key => Alpha Gating (alpha * state + no-context)
        h_gated = (alpha.unsqueeze(-1) * h_state) + h_no_context
    ...
    def forward(
        self, state: torch.Tensor, lang: torch.Tensor, alpha: torch.Tensor, action: torch.Tensor
    ) -> torch.Tensor:
    ...
    def training_step(
        ...
        state, language, alpha, action = batch

        # Get Predicted/Reconstructed Action
        predicted_action = self.forward(state, language, alpha, action)
